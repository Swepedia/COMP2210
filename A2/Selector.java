 ...
 */
   ...
   public static <T> T kmin(Collection<T> coll, int k, Comparator<T> comp)
      ...
      List<T> elements = new ArrayList<>(coll);
      java.util.Collections.<T>sort(elements, comp);
      
      int increment = 1
         ;
      T lastElement = elements.get(0);
      
      for (T i : elements) {
         if (comp.compare(lastElement, i) != 0) {
            increment++;
            lastElement = i;
         }
      }
      ...
      if (elements.size() != 1 && comp.compare(elements.get(0), elements.get(elements.size() - 1))
            < 0) {
         for (T i : elements) {
            if (increment == k) {
               return lastElement;
      ...
      if (elements.size() != 1) {
         for (int i = elements.size() - 1; i > -1; i--) {
            ...
            if (comp.compare(lastElement, elements.get(i)) != 0) {
               increment++;
